...
int readWordFile(char *fileName[], int fileLength[], char ***pppUniqueWordList, int numFiles)
{
    ...
    for (int i = 0; i < sumFileLengths; i++)
    {
...
int main()
{
    ...
    char *pFiles[1] = {"LITTLE_WOMEN.txt" /*, "MOBY_DICK.txt", "SHAKESPEARE.txt", "MIDDLEMARCH.txt"*/}; // single pointer as it is a 1D array
    int fileLengths[1] = {195467 /*, 215724, 965465, 319402*/};
    ...
    numUniqueWords = readWordFile(pFiles, fileLengths, &ppUniqueWordList, 1);
